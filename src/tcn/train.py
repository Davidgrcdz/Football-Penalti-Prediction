...
def run_training_tcn(
    ...
    # ============================================================
    loss_threshold = 0.75        # igual que MLP/LSTM
    low_loss_stop = 0.12
    ...
    for ep in range(1, epochs + 1):
        ...
        if train_loss < low_loss_stop:
            ...
            if consecutive_low >= 5:
                print(f"[TCN] -> Early stopping: pérdida muy baja ({train_loss:.6f})")
                break
        else:
            consecutive_low = 0
        ...
        if no_improve_loss >= patience_loss and no_improve_f1 >= patience_f1:
            print(f"[TCN] -> Early stopping híbrido (loss y f1 estancados)")
            early_stopped = True
            break
