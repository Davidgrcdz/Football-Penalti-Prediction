...
def run_training_transformer(
    ...
    for ep in range(1, epochs + 1):
        ...
        if ep % 10 == 0 or ep == 1:
            print(f"[TRANS] Ep {ep:03d}/{epochs} | "
        ...
        if train_loss < low_loss_stop:
            ...
            if consecutive_low >= 5:
                print(f"[TRANS] -> Early stopping: pérdida muy baja ({train_loss:.6f})")
                break
        else:
            consecutive_low = 0
        ...
        if no_improve_loss >= patience_loss and no_improve_f1 >= patience_f1:
            print(f"[TRANS] -> Early stopping híbrido (loss y f1 estancados)")
            early_stopped = True
            break
    ...
    # Si nunca hubo checkpoint válido
